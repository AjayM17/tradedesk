...
class TradeValidator {
  static const double maxPortfolioRiskPercent = 5.0;
  ...
  static TradeValidationResult validateNewTrade({
    ...
    if (trade.stopLoss <= 0 || trade.stopLoss == trade.entryPrice) {
      return const TradeValidationResult.invalid(
        'Stop-loss is mandatory',
      );
    }

    // 2️⃣ Risk per trade (R-based / planned risk)
    if (trade.totalRisk > settings.riskAmountPerTrade) {
    ...
    if (trade.investedAmount > settings.maxCapitalPerStockAmount) {
      return const TradeValidationResult.invalid(
  ...
  // =========================================================
  // PORTFOLIO RISK VALIDATION (FIXED – SL BASED)
  // =========================================================
  static TradeValidationResult validatePortfolioRisk({
    ...
    for (final trade in activeTrades) {
      if (editingTradeId != null &&
        ...
        // ✅ exclude current trade when editing
        existingQty = trade.quantity;
        continue;
      }

      // ✅ FIX: use LIVE portfolio risk from CURRENT SL
      existingRisk += trade.portfolioRisk;
    }
